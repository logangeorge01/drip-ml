...
class Predict():
    def __init__(self):
        data_path = './models/'
        cat_learner = load_learner(path=data_path, file='cat-rn50-size150-fr4-unfr2-fr4-unfr4.pkl')
        texture_learner = load_learner(path=data_path, file='texture-resnet50-size150-fr12-unfr4.pkl')
        fabric_learner = load_learner(path=data_path, file='fabric-resnet50-size150-fr5-unfr4.pkl')
        parts_learner = load_learner(path=data_path, file='parts-resnet50-size150-fr5-unfr4.pkl')
        shape_learner = load_learner(path=data_path, file='shape-resnet50-size150-fr5-unfr4.pkl')
        ...
        self.rtr_inventory.load('Logan.ann')
        self.rtr_images = glob.glob('static/*')
        #with open('logan_images.pkl', 'rb') as f:
            #self.rtr_images = pickle.load(f)
        self.rtr_df = pd.read_csv('logan.csv', index_col='img_name')
    ...
    def query_ann_index(self, embeddings, n=6):
    ...
    def get_recs(self, img_path, n=6):
